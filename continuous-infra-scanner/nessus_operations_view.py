...
def get_ip_wise_scan_data():
    # Fetch the latest insert_trackRecord value from the nessus_scan collection
...
def check_and_notify_new_ports(sorted_data):
    ...
    if previous_data:
        ...
        if new_ports:
            send_notification(new_ports, "New Ports Detected")
        if closed_ports:
            send_notification(closed_ports, "Ports Closed")



def get_ip_wise_scan_data():
    # Fetch the latest entry from the nessus_scan collection
    latest_entry = nessus_collection.find_one(sort=[("insert_trackRecord", -1)])
    if not latest_entry:
        return jsonify({'status': 'error', 'message': 'No scan data found'}), 404
...
def save_alert(port_changes, alert_type):
    ...
    for ip, ports in port_changes.items():
        ...
        print(message)    
        # Send the message to your notification system
